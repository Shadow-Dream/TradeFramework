"""Validated Analysis definition queries for outer use-case composition."""
...
from engine.compiler.analysis import validate_analysis_definition_authority
...
def analysis_definitions(config):
    definitions = []
    records = graph_resources.load_repository(config, "analysis")
    authorities = _module_authorities(config, records.values())
    for definition in records.values():
        definitions.append(
            validate_analysis_definition_authority(definition, authorities)
        )
...
def get_analysis_definition(
    ...
    return validate_analysis_definition_authority(definition, authorities)
