...
interface SessionContext {
  ...
  stderrLines: string[]
  closed: boolean
  mcpGrant: string | null
  accountLoginId?: string | null
  onAccountLoginCompleted?: (result: AccountLoginCompletedNotification) => void
}
...
export class CodexAppServerManager {
  ...
  private loginContext: SessionContext | null = null

  constructor(args: { spawnProcess?: SpawnCodexAppServer; environment?: NodeJS.ProcessEnv } = {}) {
    const environment = args.environment ? { ...args.environment } : { ...process.env }
  ...
  /** Start Codex's native app-server device-code flow; no terminal output is parsed. */
  async startDeviceLogin(
    cwd: string,
    onCompleted: (result: AccountLoginCompletedNotification) => void,
  ): Promise<{ loginId: string; verificationUrl: string; userCode: string }> {
    if (this.loginContext && !this.loginContext.closed) throw new Error("Codex sign-in is already running.")
    const child = this.spawnProcess(cwd)
    const context: SessionContext = {
      chatId: `login-${randomUUID()}`,
      cwd,
      child,
      ...
      onAccountLoginCompleted: (result) => {
        ...
        if (context.accountLoginId && result.loginId && result.loginId !== context.accountLoginId) return
        this.closeLoginContext(context)
        onCompleted(result)
      },
    }
    ...
    try {
      await this.sendRequest(context, "initialize", {
        clientInfo: { name: "trade_engine_agent_web", title: "TradeEngine Agent Web", version: "1" },
      ...
      const response = await this.sendRequest<{
        ...
        userCode: string
      }>(context, "account/login/start", { type: "chatgptDeviceCode" })
      if (response.type !== "chatgptDeviceCode" || !response.loginId || !response.verificationUrl || !response.userCode) {
        throw new Error("Codex app-server returned an invalid device-code challenge.")
      }
      context.accountLoginId = response.loginId
      return {
  ...
  async cancelDeviceLogin() {
    ...
    const loginId = context.accountLoginId
    try {
      if (loginId) await this.sendRequest(context, "account/login/cancel", { loginId })
    } finally {
      this.closeLoginContext(context)
    }
  }

  private closeLoginContext(context: SessionContext) {
    if (this.loginContext === context) this.loginContext = null
    context.closed = true
    for (const pending of context.pendingRequests.values()) pending.reject(new Error("Codex sign-in stopped."))
    context.pendingRequests.clear()
    try { context.child.kill("SIGKILL") } catch { /* already exited */ }
  }
  ...
  async readAccount(probeCwd: string): Promise<GetAccountResponse | null> {
    for (const context of this.sessions.values()) {
      if (context.closed) continue
      return await this.sendRequest<GetAccountResponse>(context, "account/read", { refreshToken: false })
    }
    return await this.probeRequest<GetAccountResponse>(probeCwd, "account/read", { refreshToken: false })
  }

  private async probeAccountRateLimits(cwd: string): Promise<GetAccountRateLimitsResponse | null> {
    return await this.probeRequest<GetAccountRateLimitsResponse>(cwd, "account/rateLimits/read", {})
  }

  private async probeRequest<TResult>(cwd: string, method: string, params: unknown): Promise<TResult | null> {
    ...
    try {
      ...
      this.writeMessage(context, { method: "initialized" })
      return await this.sendRequest<TResult>(context, method, params)
    } finally {
      context.closed = true
      try {
        child.kill("SIGKILL")
  ...
  async startSession(args: StartCodexSessionArgs) {
    ...
    const mcpGrant = args.mcpServer?.env.TRADE_ENGINE_TOOL_GRANT_FILE ?? null
    if (existing && !existing.closed && existing.cwd === args.cwd
      && existing.mcpGrant === mcpGrant && !args.pendingForkSessionToken) {
      return
    }
    ...
    await this.sendRequest(context, "initialize", {
      clientInfo: {
        name: "trade_engine_agent_web",
        title: "TradeEngine Agent Web",
        version: "1",
      },
      capabilities: {
    ...
    const mcpConfig = args.mcpServer ? {
      mcp_servers: {
        trade_engine: {
          command: args.mcpServer.command,
          args: args.mcpServer.args,
          env: args.mcpServer.env,
          startup_timeout_sec: 10,
          tool_timeout_sec: Math.ceil(args.mcpServer.timeout / 1000),
          enabled: true,
        },
      },
    } : undefined
  ...
  private async handleNotification(context: SessionContext, notification: ServerNotification) {
    ...
    if (notification.method === "account/login/completed") {
      context.onAccountLoginCompleted?.(notification.params)
      return
    }
  ...
  private handleItemCompleted(pendingTurn: PendingTurn, notification: ItemCompletedNotification) {
    ...
    if (notification.item.type === "mcpToolCall"
      && notification.item.server === "trade_engine"
      && notification.item.tool === "trade_proposal_create"
      && notification.item.status === "completed") {
      const payload = contentFromMcpResult(notification.item)
      const artifact = findReviewArtifact(payload)
      if (artifact) {
        const contextDigest = payload && typeof payload === "object" && !Array.isArray(payload)
          && typeof (payload as { contextDigest?: unknown }).contextDigest === "string"
          ? (payload as { contextDigest: string }).contextDigest
          : undefined
        pendingTurn.queue.push({
          type: "transcript",
          entry: timestamped({ kind: "review_artifact", artifact, contextDigest }),
        })
      }
    }
  }
  ...
  private failContext(context: SessionContext, message: string) {
    if (context.onAccountLoginCompleted && !context.closed) {
      context.onAccountLoginCompleted({ loginId: context.accountLoginId, success: false, error: message })
    }
    const pendingTurn = context.pendingTurn
