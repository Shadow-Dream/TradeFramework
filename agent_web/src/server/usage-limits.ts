...
function staticProviderSnapshot(provider: AgentProvider): ProviderUsageSnapshot {
  return {
    ...
    windows: [],
    credits: null,
    detail: "Usage limits have not been loaded yet.",
    updatedAt: null,
  }
}
...
const PROVIDER_ORDER: AgentProvider[] = ["claude-deepseek", "codex-openai"]
...
export class UsageLimitsManager {
  ...
  constructor(filePath: string, deps: UsageLimitsManagerDeps = {}) {
    ...
    // Both backends start "unknown" until first fetch.
    this.snapshots.set("claude-deepseek", {
      provider: "claude-deepseek", status: "unknown", plan: null, windows: [], credits: null, detail: null, updatedAt: null,
    })
    this.snapshots.set("codex-openai", {
      provider: "codex-openai", status: "unknown", plan: null, windows: [], credits: null, detail: null, updatedAt: null,
    })
  }
  ...
  recordClaudeRateLimitPush(info: ClaudeRateLimitInfoRaw) {
    const prev = this.snapshots.get("claude-deepseek") ?? null
    this.setProvider("claude-deepseek", mergeClaudeRateLimitPush(prev, info, this.nowIso()))
  }

  /** Record a pushed Codex rate-limit snapshot (sparse). */
  recordCodexRateLimitPush(raw: CodexRateLimitSnapshotRaw) {
    const prev = this.snapshots.get("codex-openai") ?? null
    this.setProvider("codex-openai", mergeCodexRateLimitPush(prev, raw, this.nowIso()))
  }
  ...
  private async refreshClaude() {
    ...
    try {
      ...
      this.applyRefreshed("claude-deepseek", normalizeClaudeUsage(raw, this.nowIso()))
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error)
      this.applyRefreshed("claude-deepseek", {
        provider: "claude-deepseek", status: "unavailable", plan: null, windows: [], credits: null,
  ...
  private async refreshCodex() {
    ...
    try {
      ...
      this.applyRefreshed("codex-openai", normalizeCodexRateLimits(raw, this.nowIso()))
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      const isAuth = /chatgpt|auth|login|sign/i.test(message)
      this.applyRefreshed("codex-openai", {
        provider: "codex-openai", status: "unavailable", plan: null, windows: [], credits: null,
        detail: isAuth
  ...
  private async persist() {
    const file: UsageLimitsFile = {
      version: 1,
      providers: {
        "claude-deepseek": this.snapshots.get("claude-deepseek"),
        "codex-openai": this.snapshots.get("codex-openai"),
      },
    }
    try {
