import type { AgentProjectKind, AgentProvider, QueuedChatMessage, TranscriptEntry } from "../shared/types"

export interface ProjectRecord {
  id: string
  /** Stable server-owned catalog identity; never a filesystem path. */
  workspaceKey: string
  title: string
...
export interface ChatRecord {
  id: string
  /** TradeEngine account that owns every operation and transcript entry. */
  ownerId: string
  /** Idempotency key used to create this chat. */
  createRequestId: string
  /** Fork source bound to createRequestId; absent/null for an ordinary chat. */
  createSourceChatId?: string | null
  projectId: string
  title: string
  createdAt: number
  updatedAt: number
  ...
  lastTurnOutcome: "success" | "failed" | "cancelled" | "interrupted" | null
  lastErrorCode?: string
  lastErrorRetryable?: boolean
  lastErrorMessage?: string
  /** The only turn allowed to mutate this chat's terminal state. */
  activeTurnId?: string
  turnRequests?: Array<{ clientRequestId: string; inputDigest: string; turnId: string }>
  /**
...
export type ProjectEvent = {
  ...
  workspaceKey: string
  title: string
  kind: NonNullable<ProjectRecord["kind"]>
} | {
  v: 4
...
export type ChatEvent =
  | {
      ...
      projectId: string
      title: string
      /** Backend is immutable from the instant the chat is created. */
...
export type StoreEvent = ProjectEvent | ChatEvent | QueuedMessageEvent | TurnEvent
