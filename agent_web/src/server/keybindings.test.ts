...
describe("KeybindingsManager", () => {
  ...
  test("uses the runtime profile for the default keybindings path", () => {
    const previous = process.env.TRADE_AGENT_RUNTIME_PROFILE
    process.env.TRADE_AGENT_RUNTIME_PROFILE = "dev"

    const manager = new KeybindingsManager()

    expect(manager.filePath).toEndWith("/.trade-agent-dev/keybindings.json")

    if (previous === undefined) {
      delete process.env.TRADE_AGENT_RUNTIME_PROFILE
    } else {
      process.env.TRADE_AGENT_RUNTIME_PROFILE = previous
    }
  })
})
