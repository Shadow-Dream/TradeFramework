...
const originalRuntimeProfile = process.env.TRADE_AGENT_RUNTIME_PROFILE
const tempDirs: string[] = []

afterEach(async () => {
  if (originalRuntimeProfile === undefined) {
    delete process.env.TRADE_AGENT_RUNTIME_PROFILE
  } else {
    process.env.TRADE_AGENT_RUNTIME_PROFILE = originalRuntimeProfile
  }
...
async function registerTestProject(store: EventStore, projectId = "trade-engine") {
  return await store.registerProject({
    projectId,
    workspaceKey: projectId,
    title: projectId === "trade-engine" ? "TradeEngine" : projectId,
    kind: projectId === "trade-engine" ? "trade-engine" : "strategy",
  })
}
...
describe("EventStore", () => {
  test("uses the runtime profile for the default data dir", () => {
    ...
    expect(store.dataDir).toEndWith("/.trade-agent-dev/data")
  })
  ...
  test("appends new transcript entries only to the per-chat transcript file", async () => {
    ...
    const project = await registerTestProject(store)
    ...
    expect(existsSync(join(dataDir, "messages.jsonl"))).toBe(false)
  ...
  test("marks chats unread on completed turns and clears unread when marked read", async () => {
    ...
    const finishedTurnId = await store.recordTurnStarted(chat.id)
    await store.recordTurnFinished(chat.id, finishedTurnId)
    expect(store.getChat(chat.id)?.unread).toBe(true)

    await store.setChatReadState(chat.id, false)
    expect(store.getChat(chat.id)?.unread).toBe(false)

    const failedTurnId = await store.recordTurnStarted(chat.id)
    await store.recordTurnFailed(chat.id, failedTurnId, "boom")
    expect(store.getChat(chat.id)?.unread).toBe(true)

    const cancelledTurnId = await store.recordTurnStarted(chat.id)
    await store.recordTurnCancelled(chat.id, cancelledTurnId)
    expect(store.getChat(chat.id)?.unread).toBe(true)
  ...
  test("preserves read state after a finished turn across restart", async () => {
    ...
    const turnId = await store.recordTurnStarted(chat.id)
    await store.recordTurnFinished(chat.id, turnId)
    await store.setChatReadState(chat.id, false)
  ...
  test("preserves read state after a failed turn across restart", async () => {
    ...
    const turnId = await store.recordTurnStarted(chat.id)
    await store.recordTurnFailed(chat.id, turnId, "boom")
    await store.setChatReadState(chat.id, false)
  ...
  test("persists sidebar project order across restart and compaction", async () => {
    ...
    const first = await registerTestProject(store, "strategy:a")
    const second = await registerTestProject(store, "strategy:b")
  ...
  test("renames only the logical sidebar title", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    ...
    expect(store.getProject(project.id)?.title).toBe("TradeEngine")
    expect(store.getProject(project.id)?.sidebarTitle).toBe("Sidebar Name")
    expect(store.getProject(project.id)?.workspaceKey).toBe("trade-engine")
    expect(store.getProject(project.id)).not.toHaveProperty("localPath")

    const reloaded = new EventStore(dataDir)
    await reloaded.initialize()

    expect(reloaded.getProject(project.id)?.title).toBe("TradeEngine")
    expect(reloaded.getProject(project.id)?.sidebarTitle).toBe("Sidebar Name")
    expect(reloaded.getProject(project.id)?.workspaceKey).toBe("trade-engine")
  ...
  test("forks a chat with copied transcript and pending fork session token", async () => {
    ...
    expect(forked.provider).toBe("claude-deepseek")
  ...
  test("a fork inherits lastTurnEndedAt, so it keeps the conversation's recency", async () => {
    ...
    const turnId = await store.recordTurnStarted(source.id)
    await store.recordTurnFinished(source.id, turnId)
  ...
  test("reopening a removed project restores its existing chats", async () => {
    ...
    const reopened = await registerTestProject(store)
  ...
  test("counts turns, and survives the replay that would double-count them", async () => {
    ...
    const firstTurnId = await store.recordTurnStarted(chat.id)
    await store.appendMessage(chat.id, entry("user_prompt", 1_000, { content: "first" }))
    await store.recordTurnFinished(chat.id, firstTurnId)
    const secondTurnId = await store.recordTurnStarted(chat.id)
    await store.appendMessage(chat.id, entry("user_prompt", 2_000, { content: "second" }))
    await store.recordTurnFinished(chat.id, secondTurnId)
  ...
  test("recovers an unmatched turn as interrupted while preserving the native session", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()

    const project = await registerTestProject(store)
    const chat = await store.createChat(project.id, "local-test-owner", "codex")
    await store.setSessionToken(chat.id, "native-thread-1")
    const turnId = await store.recordTurnStarted(chat.id, "gpt-5.4")
    await store.appendMessage(chat.id, {
      _id: "tool-before-crash",
      createdAt: Date.now(),
      kind: "tool_call",
      tool: {
        kind: "tool",
    ...
    const recoveredChat = recovered.requireChat(chat.id)
    expect(recoveredChat.activeTurnId).toBeUndefined()
    expect(recoveredChat.lastTurnOutcome).toBe("interrupted")
    expect(recoveredChat.lastTurnEndedAt).toBeNumber()
    expect(recoveredChat.sessionToken).toBe("native-thread-1")
    expect(recoveredChat.unread).toBe(true)

    const interruption = recovered.getMessages(chat.id).filter((entry) => (
      entry.kind === "interrupted" && entry.reason === "service_restart"
    ))
    expect(interruption).toHaveLength(1)
    expect(interruption[0]).toMatchObject({ turnId })

    // Recovery is retry-safe and never manufactures duplicate transcript rows.
    const reloadedAgain = new EventStore(dataDir)
    await reloadedAgain.initialize()
    expect(reloadedAgain.getMessages(chat.id).filter((entry) => (
      entry.kind === "interrupted" && entry.reason === "service_restart"
    ))).toHaveLength(1)

    // Continuing creates a new turn on the same native session; it never
    // replays or reopens the abandoned turn.
    const nextTurnId = await reloadedAgain.recordTurnStarted(chat.id, "gpt-5.4")
    expect(nextTurnId).not.toBe(turnId)
    expect(reloadedAgain.requireChat(chat.id).sessionToken).toBe("native-thread-1")
    await reloadedAgain.recordTurnFinished(chat.id, nextTurnId)
    expect(reloadedAgain.requireChat(chat.id).lastTurnOutcome).toBe("success")
  })

  test("rejects overlapping starts and ignores a stale terminal turn id", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    ...
    const project = await registerTestProject(store)
    const chat = await store.createChat(project.id)
    const activeTurnId = await store.recordTurnStarted(chat.id)

    await expect(store.recordTurnStarted(chat.id)).rejects.toThrow("already has an active turn")
    expect(await store.recordTurnFinished(chat.id, crypto.randomUUID())).toBe(false)
    expect(store.requireChat(chat.id).activeTurnId).toBe(activeTurnId)
    expect(store.requireChat(chat.id).lastTurnOutcome).toBeNull()

    expect(await store.recordTurnFinished(chat.id, activeTurnId)).toBe(true)
    expect(store.requireChat(chat.id).activeTurnId).toBeUndefined()
  })
  ...
  test("a fork inherits the turns behind the conversation it copied", async () => {
    ...
    const source = await store.createChat(project.id)
    await store.setSessionToken(source.id, "session-1")
    const firstTurnId = await store.recordTurnStarted(source.id)
    await store.appendMessage(source.id, entry("user_prompt", 1_000, { content: "first" }))
    await store.recordTurnFinished(source.id, firstTurnId)
    const secondTurnId = await store.recordTurnStarted(source.id)
    await store.recordTurnFinished(source.id, secondTurnId)
