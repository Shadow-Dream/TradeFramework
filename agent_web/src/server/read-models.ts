...
export function deriveStatus(chat: ChatRecord, activeStatus?: KannaStatus): KannaStatus {
  ...
  if (chat.activeTurnId) return "starting"
  if (chat.lastErrorCode === "reauth_required") return "reauth_required"
  if (chat.lastTurnOutcome === "interrupted") return "interrupted"
...
export function deriveSidebarData(
  ...
  for (const chat of state.chatsById.values()) {
    if (options?.ownerId && chat.ownerId !== options.ownerId) continue
    ...
    return projectChats
      ...
      .map((chat) => {
        ...
        return {
          ...
          ...(chat.lastErrorCode ? { errorCode: chat.lastErrorCode } : {}),
          ...(chat.lastErrorRetryable != null ? { errorRetryable: chat.lastErrorRetryable } : {}),
          ...(chat.lastErrorMessage ? { errorMessage: chat.lastErrorMessage } : {}),
...
export function deriveLocalProjectsSnapshot(
  ...
  for (const project of [...state.projectsById.values()].filter((entry) => !entry.deletedAt)) {
    const chats = [...state.chatsById.values()].filter((chat) => (
...
export function deriveChatSnapshot(
  ...
  if (!chat || chat.deletedAt || (ownerId && chat.ownerId !== ownerId)) return null
  ...
  const currentTool = (status === "starting" || status === "running" || status === "waiting_for_user")
    ? [...transcript.messages].reverse().find((entry) => entry.kind === "tool_call")?.tool.toolName
    : undefined
  ...
  const runtime: ChatRuntime = {
    ...
    ...(chat.lastTurnStartedAt != null ? { turnStartedAt: chat.lastTurnStartedAt } : {}),
    ...(chat.updatedAt != null ? { lastEventAt: chat.updatedAt } : {}),
  ...
  return {
    ...
    availableProviders: SERVER_PROVIDERS.filter((provider) => provider.id === "claude-deepseek" || provider.id === "codex-openai"),
