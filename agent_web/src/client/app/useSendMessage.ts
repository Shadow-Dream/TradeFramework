...
export function useSendMessage(params: {
  ...
  const handleSend = useCallback(async (
    ...
    const { isProcessing, optimisticUserPrompts, serverTranscriptEntries, selectedProjectId } = sendContextRef.current
    const attachments = options?.attachments ?? []
    const clientRequestId = generateUUID()
    const composerState = useChatPreferencesStore.getState().getComposerState(activeChatId ?? NEW_CHAT_COMPOSER_ID)
    const provider = options?.provider ?? composerState.provider
    const model = options?.model ?? composerState.model
    ...
    try {
      ...
      if (!activeChatId && !projectId) {
        throw new Error("Select an approved Project first")
      }
