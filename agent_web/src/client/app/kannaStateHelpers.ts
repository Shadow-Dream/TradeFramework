...
export function composerStateFromSendOptions(options?: {
  ...
  if (options?.provider === "claude-deepseek" && options.model && options.modelOptions?.claude) {
    return {
  ...
  if (options?.provider === "codex-openai" && options.model && options.modelOptions?.codex) {
    return {
