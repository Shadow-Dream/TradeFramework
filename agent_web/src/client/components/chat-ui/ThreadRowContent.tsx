...
import { CircleSlash, Loader2, MessageCircle, PencilLine } from "lucide-react"
...
export function renderChatStatusDot(chat: SidebarChatRow): ReactNode | null {
  ...
  if (chat.status === "interrupted" || chat.status === "reauth_required") {
    return <CircleSlash className="size-3.5 shrink-0 text-amber-500" />
  }
...
export function ThreadRowContent({
  ...
  const needsAttention = (thread.row.status !== "idle" && thread.row.status !== "completed") || thread.row.unread
