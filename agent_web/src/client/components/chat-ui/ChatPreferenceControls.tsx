  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align={align} className="w-64 overflow-hidden p-0">
        {/* Runtime model lists can be long — scroll instead of overflowing the viewport. */}
...
interface ChatPreferenceControlsProps {
  ...
  /** Opens the Default Models dialog when a provider offers that action. */
  mode?: ChatMode
...
export function ChatPreferenceControls({
  ...
  const selectedProviderLabel = providerConfig?.label ?? selectedProvider
  const ProviderIcon = PROVIDER_ICONS[selectedProvider]
  const ModelIcon = Box
  const codexModelOptions = selectedProvider === "codex-openai" ? modelOptions as CodexModelOptions : null
