...
import { LiveUiContextPopover } from "./LiveUiContextPopover"
...
interface Props {
  ...
  /** Logical Project label; never a host filesystem path. */
...
const ChatInputInner = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const { composerChatId, selectedProvider } = composer
  ...
  const uiContext = useUiContextStore((state) => state.snapshot)
  useEffect(() => {
  ...
  const activeContextWindow = useMemo(() => {
    if (providerPrefs.provider !== "claude-deepseek") {
      return contextWindowSnapshot
    }

    const claudeModelOptions = providerPrefs.modelOptions as Extract<ComposerState, { provider: "claude-deepseek" }>["modelOptions"]
  ...
  const skillMenuTriggers = selectedProvider === "codex-openai" ? CODEX_SKILL_MENU_TRIGGERS : DEFAULT_SKILL_MENU_TRIGGERS
    ...
    <div>
      <div className={cn("px-3 pt-0", isStandalone && "px-5")}>
        <div className="relative max-w-[840px] mx-auto rounded-[32px]">
          <LiveUiContextPopover snapshot={uiContext} />
          {skillMenuOpen ? (
            <div
      ...
      <div className={cn("relative py-3 max-w-[840px] mx-auto", isStandalone && "pt-3 pb-5")}>
        <div className="overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden flex flex-row">
          ...
          <ChatPreferenceControls
            ...
            providerLocked={activeProvider !== null}
            model={providerPrefs.model}
