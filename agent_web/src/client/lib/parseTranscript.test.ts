...
describe("processTranscriptMessages", () => {
  test("preserves crash interruption identity and reason", () => {
    const messages = processTranscriptMessages([
      entry({ kind: "interrupted", reason: "service_restart", turnId: "turn-1" }),
    ])
  ...
  test("preserves attachments on hydrated user prompts", () => {
    ...
    expect(messages[0].attachments?.[0]?.relativePath).toBe("./.trade-agent/uploads/spec.pdf")
  })
