...
function getAllowedHosts() {
  ...
  const configured = process.env.TRADE_AGENT_DEV_ALLOWED_HOSTS
...
function getBackendTargetHost() {
  return process.env.TRADE_AGENT_DEV_BACKEND_TARGET_HOST || "127.0.0.1"
}

function getBackendPort() {
  const configured = Number(process.env.TRADE_AGENT_DEV_BACKEND_PORT)
