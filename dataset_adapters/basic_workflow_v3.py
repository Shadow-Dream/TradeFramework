"""Publisher for causal Basic Workflow v3 OHLCV CSV Datasets."""
    ...
    for period, details in report["periods"].items():
        ...
        for instrument in details["instruments"]:
            ...
            shutil.copyfile(
                ...
                destination_period / f"{instrument}.csv",
            )
...
def register_dataset(
    ...
    *,
    ...
    descriptor = require_basic_workflow_v3_descriptor(copy.deepcopy(descriptor))
    report = validate_dataset_directory(source_root, descriptor)
    dataset_id = resource_ids.normalize_resource_id(dataset_id)
    if type(name) is not str or not name.strip():
        raise ValueError("Basic Workflow v3 Dataset name must be a non-empty string.")
    if not isinstance(source, Mapping) or set(source) != {"type", "details"}:
        raise ValueError("Basic Workflow v3 Dataset source must use type/details.")
    if (
        ...
        raise ValueError(
            "Basic Workflow v3 Dataset source requires a non-empty type and object details."
        )
    if metadata is None:
        metadata = {}
    if not isinstance(metadata, Mapping):
        raise ValueError("Basic Workflow v3 Dataset metadata must be an object.")
    if (
        ...
        raise ValueError(
            "Basic Workflow v3 display_time_zone must be a non-empty string."
        )
    ...
    if conflicts:
        raise ValueError(
            "Basic Workflow v3 metadata conflicts with protocol field(s): "
            + ", ".join(conflicts)
        )
    ...
    try:
        ...
        return dataset_publication.publish_dataset_version(
            ...
            version_source={
                "type": "basic-workflow-v3-csv-directory",
                "details": {
                    "protocolId": descriptor["protocolId"],
                    "protocolVersion": descriptor["protocolVersion"],
                    "profile": descriptor["profile"],
