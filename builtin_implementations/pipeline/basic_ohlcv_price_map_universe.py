...
class BasicOhlcvPriceMapUniverse(UniverseModule):
    def update(self, price):
        if type(price) is not dict:
            raise ValueError("Basic Workflow OHLCV price must be an object map.")
        ...
        if type(prices) is not dict:
            raise ValueError("Basic Workflow decisionPeriod is absent from OHLCV price.")
