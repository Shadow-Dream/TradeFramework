...
namespace QuantConnect.Algorithm.Framework.Portfolio
{
    ...
    /// </summary>
    public class PortfolioConstructionModel : IPortfolioConstructionModel, IPortfolioIntentModel, IModule
    {
        ...
        /// <summary>
        /// Merge priority used when multiple target modules emit intents for the same symbol.
        /// </summary>
        ...
        {
            _moduleState = new ModuleState(GetType(), ModuleKind.Target, ModuleHotSwapMode.Live);
        ...
        /// <summary>
        /// Adapts the legacy target output of portfolio construction models into a mergeable intent stream.
        /// </summary>
        public virtual IEnumerable<PortfolioTargetIntent> CreateIntents(QCAlgorithm algorithm, Insight[] insights)
        {
            foreach (var target in CreateTargets(algorithm, insights))
            {
                yield return new PortfolioTargetIntent(target.Symbol, target.Quantity, GetType().Name, Priority, target.Tag);
            }
        }
