...
{
    internal sealed class TransportSignalModule : IAlphaModel, IModule, IDisposable
    {
        ...
        public TransportSignalModule(IModuleTransportClient client, ModuleActivationMode activationMode)
        {
        ...
        public ModuleKind Kind => ModuleKind.Signal;
        ...
        {
            var request = TransportProtocol.SerializeTime(algorithm);
            request["moduleKey"] = Key;
            var payload = _client.InvokeAsync("update_signal", request).GetAwaiter().GetResult();
            if (payload["marker"] != null)
            {
                Log.Trace((string)payload["marker"]);
            }
            return TransportProtocol.DeserializeInsights(payload);
        }
