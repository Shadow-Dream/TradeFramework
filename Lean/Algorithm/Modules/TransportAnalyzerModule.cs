...
{
    internal sealed class TransportAnalyzerModule : IAnalyzerModule, IDisposable
    {
        ...
        public TransportAnalyzerModule(IModuleTransportClient client, ModuleActivationMode activationMode)
        {
        ...
        public ModuleKind Kind => ModuleKind.Analyzer;
        ...
        public IReadOnlyCollection<ObservationKey> RequiredObservations { get; private set; } = Array.Empty<ObservationKey>();
        ...
        public async ValueTask Initialize(ModuleConfiguration configuration, CancellationToken cancellationToken = default)
        {
            ...
            var payload = await _client.InvokeAsync("initialize", new JObject
            {
                ["configuration"] = TransportProtocol.SerializeConfiguration(configuration)
            }, cancellationToken).ConfigureAwait(false);

            if (payload["requiredObservations"] is JArray required)
            {
                var keys = new List<ObservationKey>(required.Count);
                foreach (var item in required)
                {
                    if (ObservationKey.TryParse((string)item, out var key))
                    {
                        keys.Add(key);
                    }
                }
                RequiredObservations = keys;
            }
        }
        ...
        {
            var request = TransportProtocol.SerializeObservations(observations);
            request["moduleKey"] = Key;
            var payload = _client.InvokeAsync("analyze", request).GetAwaiter().GetResult();
            if (payload["marker"] != null)
            {
                Log.Trace((string)payload["marker"]);
            }
            return TransportProtocol.DeserializeAnalyzerResult(payload);
        }
