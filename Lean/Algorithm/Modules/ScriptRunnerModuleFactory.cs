...
{
    public sealed class ScriptRunnerModuleFactory : IModuleFactory
    {
        ...
        public bool CanCreate(ModuleConfiguration configuration)
        {
            return configuration != null &&
                   configuration.ActivationMode == ModuleActivationMode.ScriptRunner &&
        ...
        public ValueTask<IModule> CreateAsync(ModuleConfiguration configuration, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new NotSupportedException($"Module activation mode '{configuration?.ActivationMode}' is not supported by {nameof(ScriptRunnerModuleFactory)}.");
            }
