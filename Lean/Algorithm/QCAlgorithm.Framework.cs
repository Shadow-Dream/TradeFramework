...
namespace QuantConnect.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        private void ProcessInsights(Insight[] insights)
        {
            ...
            if (PortfolioConstruction is IPortfolioIntentModel intentModel)
            {
                var intentsEnumerable = intentModel.CreateIntents(this, insights);
                var intents = intentsEnumerable == Enumerable.Empty<PortfolioTargetIntent>()
                    ? [] : intentsEnumerable.ToArray();
                targets = intents.Length == 0
                    ? []
                    : PriorityPortfolioIntentMerger.Instance.Merge(this, intents).ToArray();
            }
            else
            {
                ...
                // for performance only call 'ToArray' if not empty enumerable (which is static)
                targets = targetsEnumerable == Enumerable.Empty<IPortfolioTarget>()
