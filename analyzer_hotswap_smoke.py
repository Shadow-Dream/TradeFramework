...
REMOTE_SERVER = ROOT / "remote_signal_server.py"
SCRIPT_TARGET = ROOT / "script_target_runner.py"
SCRIPT_ANALYZER = ROOT / "script_analyzer_runner.py"
...
def manifest(plugin_dll: Path, base_url: str, strategy: str) -> dict:
    prefix = f"Strategy{strategy}"
    return {
        "name": f"analyzer-hotswap-{strategy.lower()}",
        "modules": [
            {
                "key": "data.main",
                "kind": "Data",
            ...
            },
            {
                ...
                "entryPoint": f"remote.signal.{strategy.lower()}",
                "parameters": {"baseUrl": base_url, "strategy": strategy, "role": "signal"},
                "hotSwapMode": "Live",
            },
            {
                "key": "target.main",
                ...
                "entryPoint": f"script.target.{strategy.lower()}",
                "parameters": {"command": "python3", "arguments": str(SCRIPT_TARGET), "workingDirectory": str(ROOT), "strategy": strategy, "role": "target"},
                "hotSwapMode": "Live",
            },
            {
                "key": "risk.main",
                "kind": "Constraint",
                "activationMode": "RemoteService",
                "entryPoint": f"remote.risk.{strategy.lower()}",
                "parameters": {"baseUrl": base_url, "strategy": strategy, "role": "risk"},
                "hotSwapMode": "Live",
            },
            {
                "key": "execution.main",
                "kind": "Execution",
                "activationMode": "ScriptRunner",
                "entryPoint": f"script.execution.{strategy.lower()}",
                "parameters": {"command": "python3", "arguments": str(SCRIPT_TARGET), "workingDirectory": str(ROOT), "strategy": strategy, "role": "execution"},
            ...
            },
            {
                ...
                "kind": "MarketRule",
                "activationMode": "RemoteService",
                "entryPoint": f"remote.market.{strategy.lower()}",
                "parameters": {"baseUrl": base_url, "strategy": strategy, "role": "market"},
            ...
            },
            {
                ...
                "kind": "Analyzer",
                "activationMode": "RemoteService",
                "entryPoint": f"remote.analyzer.{strategy.lower()}",
                "parameters": {"baseUrl": base_url, "strategy": strategy, "role": "analyzer"},
                "hotSwapMode": "RequiresPause",
            },
            {
                "key": "analyzer.script",
                "kind": "Analyzer",
                "activationMode": "ScriptRunner",
                "entryPoint": f"script.analyzer.{strategy.lower()}",
                "parameters": {"command": "python3", "arguments": str(SCRIPT_ANALYZER), "workingDirectory": str(ROOT), "strategy": strategy},
                "hotSwapMode": "RequiresPause",
            },
        ],
        "data": ["data.main"],
        ...
        "analyzer": ["analyzer.remote", "analyzer.script"],
    }
...
def main():
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="lean-analyzer-hotswap-"))
    ...
    server = subprocess.Popen([sys.executable, str(REMOTE_SERVER), str(port)], cwd=ROOT)

    try:
        time.sleep(0.5)
        write_json_atomic(manifest_path, manifest(plugin_dll, base_url, "A"))
        write_json_atomic(config_path, config(algorithm_dll, manifest_path))

        flags = {"swapped_to_b": False, "swapped_to_a": False, "reload_count": 0}
        markers = {
            "REMOTE_SIGNAL_A": False,
            "SCRIPT_TARGET_A": False,
            "REMOTE_RISK_A": False,
            "SCRIPT_EXEC_A": False,
            "REMOTE_MARKET_A": False,
            "REMOTE_ANALYZER_A": False,
            "SCRIPT_ANALYZER_A": False,
            "REMOTE_SIGNAL_B": False,
            "SCRIPT_TARGET_B": False,
            "REMOTE_RISK_B": False,
            "SCRIPT_EXEC_B": False,
            "REMOTE_MARKET_B": False,
        }
        lock = threading.Lock()

        def swap_later(target: str, delay: float):
            time.sleep(delay)
            write_json_atomic(manifest_path, manifest(plugin_dll, base_url, target))
        ...
        print(f"ANALYZER_HOTSWAP_SMOKE_OK log={output_path}", flush=True)
    finally:
